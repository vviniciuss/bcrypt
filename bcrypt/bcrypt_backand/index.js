...
app.post('/logar', async(req,res)=>{
    ...
    try{
        const pesquisa = await Usuario.findOne({where: {email: login.email}, raw:true})
        console.log(pesquisa)
        
        if(pesquisa === null){
            console.log("Usuário não existe no banco de dados!")
            res.status(404).json({message: "Usuário não existe no banco de dados!"})
        }else if(pesquisa.email == login.email){

            bcrypt.compare(login.senha, pesquisa.senha, (err, result)=> {
                if (err) {
...
app.post('/cadastrar', (req, res) => {
    ...
    bcrypt.hash(dados.senha, 10, async(err, hash) => {
        ...
        try {
            ...
            {nome: dados.nome, email: dados.email, senha: hash })    
            console.log(grav)    
